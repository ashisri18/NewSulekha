 ...
 *  com.kirwa.screenrecorder.Recorder
 *  org.testng.IClass
...
public class MyNXGTestListner extends NXGTestListner{
	...
	public void onTestStart(ITestResult testResult) {
		...
		if (TestDirectory.recordTestExecution) {
			try {
				//this.testRecorder = new Recorder(String.valueOf(NXGTestListner.getReportDir(testResult)) + TestDirectory.SEP + testResult.getName() + ".mov");
				//this.testRecorder.startRecording();
			}
			catch (Exception e) {
				e.printStackTrace();
			}
			System.out.println("started recording for " + NXGTestListner.getReportDir(testResult) + TestDirectory.SEP + testResult.getName() + ".mov");
		}
	}
	...
	public void onStart(ISuite suite) {
		if (!this.isSuiteStarted) {
			...
			try {
				...
				if (TestDirectory.recordSuiteExecution) {
					//this.recorder = new Recorder(String.valueOf(TestDirectory.RUNDIR) + TestDirectory.SEP + "suite.mov");
...
public void generateIndexPage() {
		...
		try {
			try {
				...
				IndexPageWriter.printContent(printWriter, "Graphical representation of multiple execution cycle");
